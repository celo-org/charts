...
{{- $_ := required "op-node.secrets.l1Url.value is required: provide a clusters/<cluster>.yaml file via -f, or override with --set" $node.secrets.l1Url.value -}}
{{- $_ := required "op-node.secrets.l1BeaconUrl.value is required: provide a clusters/<cluster>.yaml file via -f, or override with --set" $node.secrets.l1BeaconUrl.value -}}
